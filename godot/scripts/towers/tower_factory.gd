...
func tower_factory():
	...
	var shuriken_projectile = ProjectileDef.new().Display("Shuriken").Pierce(2).Radius(4).EffectMask([Bloon.BloonImmunity.IMMUNITY_NONE]).DamageEffect(DamageEffectDef.new().Damage(1).CantBreak([Bloon.BloonType.LEAD]).CanBreakIce(false))
