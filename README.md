# btd5-restored

The original **Flash** version of *Bloons Tower Defense 5* rewritten in Godot 4!

The main goal of this project is to bring back the old vector artwork that was sadly scrapped in favor of the new png art for the Steam/mobile version of BTD5. It also comes with quality of life features and much better performance than the original flash version.

**Disclaimer:** this is an unofficial fan project. It is not endorsed by Ninja Kiwi.\
This repo **includes Ninja Kiwi's original Flash assets** (graphics & audio). They have been ripped from the Flash release using [ffdec](https://github.com/jindrapetrik/jpexs-decompiler).
